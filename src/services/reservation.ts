...
export class ReservationService {
      ...
      if (theaterId) {
        ...
        if (context.user.role === UserRole.THEATER_ADMIN) {
          ...
          if (!theaterExists) {
            ...
            throw new GraphQLError('Unauthorized: Theater not found or you are not the admin.');
          }
        }
      }
  ...
  static async getReservationsByUser(_, { userId }: { userId: string }, context) {
    restrictRole(context, [UserRole.THEATER_ADMIN]);

    try {
      const userReservation = await Reservation.find({ userId }).populate([userId]);
      return userReservation;
    } catch (error) {
      logger.error(`Error in finding reservation by ID ${error.message}`);
      throw new GraphQLError(`Error in finding reservation by ID ${error.message}`);
    }
  }
  ...
  static async cancelReservation(_, { id }: { id: string }, context) {
    ...
    try {
      ...
      const reservation = await Reservation.findById(id).select('userId showId isDeleted');
      ...
      if (user.role === UserRole.CUSTOMER && reservation.userId.toString() !== user.id) {
        logger.warn(`Unauthorized attempt by user ${user.id} to cancel reservation ${id}`);
      ...
      if (user.role === UserRole.THEATER_ADMIN) {
        const show = await Show.findById(reservation.showId).select('theaterId');

        if (!show) {
          logger.error(`Show ${reservation.showId} not found for reservation ${id}`);
          throw new GraphQLError('Show not found for this reservation');
        }

        const theater = await Theater.findById(show.theaterId).select('adminId');

        if (!theater) {
          logger.error(`Theater ${show.theaterId} not found for reservation ${id}`);
